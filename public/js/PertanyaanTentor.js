...
var pertanyaantable = $("#datapertanyaantentor").DataTable({
    ...
    responsive: true,
    ajax: {
        url: "/datapertanyaantentor/data/" + pageID,
        type: "GET",
        headers: {
        ...
        { data: "tipe_pertanyaan", name: "tipe_pertanyaan" },
        {
            ...
            render: function (data, type, full, meta) {
                return (
                    '<button value="' +
                    data +
                    '" class="btn btn-xs btn-primary edit_datapertanyaantentor"><i class="fa fa-edit"></i></button>'
                );
            },
            orderable: false,
            searchable: false,
        },
        {
            ...
            render: function (data, type, full, meta) {
                return (
                    '<button value="' +
                    data +
                    '" class="btn btn-xs btn-danger delete_datapertanyaantentor"><i class="fa fa-trash"></i></button>'
                );
            },
            orderable: false,
            searchable: false,
...
/* Tambah Data */
$(function () {
    $("#datapertanyaantentor_form").on("submit", function (e) {
        e.preventDefault();
        $.ajaxSetup({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            ...
            success: function (response) {
                ...
                if (response.status == 400) {
                    ...
                    if ($("#tipe_pertanyaan").val() == "Pilihan") {
                        $("#datapertanyaantentor_form")[0].reset();
                    }
                    $("#datapertanyaantentor_form").find("textarea").val("");
...
/* Edit Data */
$(document).on("click", ".edit_datapertanyaantentor", function (e) {
    e.preventDefault();
    var pertanyaan_id = $(this).val();
    $("#editpertanyaanmodal").modal("show");
    $.ajax({
        type: "GET",
        url: "/datapertanyaantentor/edit/" + pertanyaan_id,
        success: function (response) {
            console.log(response);
...
// /* Update Data */
$(document).on("submit", "#datapertanyaantentoredit_form", function (e) {
    e.preventDefault();
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
    ...
    let EditformData = new FormData($("#datapertanyaantentoredit_form")[0]);

    $.ajax({
        type: "POST",
        url: "/datapertanyaantentor/update/" + pertanyaan_id,
        data: EditformData,
        contentType: false,
        processData: false,
        beforeSend: function () {
...
$(document).on("click", ".delete_datapertanyaantentor", function (e) {
    e.preventDefault();
    var pertanyaan_id = $(this).val();
    ...
    swalWithBootstrapButtons
        ...
        .then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "/datapertanyaantentor/delete/" + pertanyaan_id,
                    success: function (response) {
                        console.log(response);
                        pertanyaantable.ajax.reload();
                    },
